...
function Cart() {
    ...
    const finalPrice = context.cart.totalPrice * 1.03
    const checkout = async () => {
        ...
        try {
            ...
            const purchaseId = (await purchaseResponse.json())._id
            
            const ordersItems = {}
            context.cart.items.forEach((item) => {
                ordersItems[item.product.artisan] ? ordersItems[item.product.artisan].push(item) : ordersItems[item.product.artisan] = [item];
            })
            
            Object.keys(ordersItems).forEach(async artisanId => {
                const order = {
                    price: finalPrice,
                    orderStatus: "Pending",
                    media: ordersItems[artisanId].flatMap(item => item.product.media.photos),
                    orderItems: ordersItems[artisanId],
                    artisan: artisanId,
                    purchase: purchaseId,
                    client: _id
                }
                
                const orderResponse = await fetch(`${process.env.REACT_APP_BACKEND_URL}/orders/?token=${process.env.REACT_APP_TOKEN}`, {
                    method: 'POST',
                    mode: 'cors',
                    headers: {
                        'Content-Type': 'application/json',
                        ...
                        <div className='row cart-table-summary-row'>
                            ...
                            <div className="col-12 col-lg-3 cart-summary-col">
                                ...
                                <p className="d-flex justify-content-between bd-highlight mb-3 text "
                                    ...
                                    <span className="bd-highlight text-uppercase"><FormattedMessage id="Total" /></span><span className="bd-highlight">$<FormattedNumber value={finalPrice} /></span>
                                </p>
                                <button type="submit"
                                    className="btn text-uppercase cart-button"
                                    onClick={() => {
